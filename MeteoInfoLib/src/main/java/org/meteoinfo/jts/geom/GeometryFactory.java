...
import org.meteoinfo.jts.geom.impl.*;
...
public class GeometryFactory
     ...
     * Converts the <code>List</code> to an array.
     *
     * @param multiLineStrings the <code>List</code> of MultiLineStrings to
     * convert
     * @return the <code>List</code> in array format
     */
     ...
     * The Geometry returned is guaranteed to be valid. To provide this
     * behaviour, the following cases occur:
     * <p>
     ...
     * <li>a rectangle : returns a {@link Polygon}> whose points are (minx,
    ...
    /**
     * Returns the PrecisionModel that Geometries created by this factory will
     * be associated with.
     *
     ...
     *
     * @param geometries an array of Geometries, each of which may be empty but
     * not null, or null
    ...
    /**
     * Creates a {@link LinearRing} using the given {@link Coordinate}s. A null
     * or empty array creates an empty LinearRing. The points must form a closed
     * and simple linestring.
     *
     * @param coordinates an array without null elements, or an empty array, or
     * null
     * @return the created LinearRing
     * @throws IllegalArgumentException if the ring is not closed, or has too
     * few points
     */
    public LinearRing createLinearRing(Coordinate[] coordinates) {
        if (!coordinates[0].equals2D(coordinates[coordinates.length - 1])) {
            coordinates = (Coordinate[]) DataConvert.resizeArray(coordinates, coordinates.length + 1);
     ...
     * A null or empty array creates an empty LinearRing. The points must form a
    ...
    /**
     * Creates a {@link MultiPoint} using the given {@link Point}s. A null or
     * empty array will create an empty MultiPoint.
     *
     * @param point an array of Points (without null elements), or an empty
    ...
    /**
     * Creates a {@link MultiPoint} using the given {@link Coordinate}s. A null
     * or empty array will create an empty MultiPoint.
     *
     ...
     * {@link CoordinateSequence}. A <code>null</code> or empty
     * CoordinateSequence creates an empty MultiPoint.
     *
     * @param coordinates a CoordinateSequence (possibly empty), or
     * <code>null</code>
     ...
     * </ul>
     *
     * Note that this method does not "flatten" Geometries in the input, and
     * hence if any MultiGeometries are contained in the input a
     ...
     * {@link CoordinateSequenceFactory} defined for this factory is used to
     * copy the {@link CoordinateSequence}s of the input geometry.
     * <p>
     * This is a convenient way to change the <tt>CoordinateSequence</tt>
     * used to represent a geometry, or to change the factory used for a
     * geometry.
     * <p>
     * {@link Geometry#clone()} can also be used to make a deep copy, but it
     * does not allow changing the CoordinateSequence type.
     *
     * @return a deep copy of the input geometry, using the CoordinateSequence
