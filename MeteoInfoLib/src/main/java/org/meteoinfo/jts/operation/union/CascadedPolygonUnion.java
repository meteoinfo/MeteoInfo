 ...
 */
package org.meteoinfo.jts.operation.union;
 ...
 * geometrys. The geometries are indexed using a spatial index, and unioned
 ...
 * can sometimes take a long time to complete. This is particularly the case
 * where there is a high degree of overlap between the polygons. In this case,
 * <tt>buffer(0)</tt> is forced to compute with <i>all</i> line segments from
 * the outset, whereas cascading can eliminate many segments at each stage of
 * processing. The best situation for using <tt>buffer(0)</tt> is the trivial
 * case where there is <i>no</i> overlap between the input geometries. However,
 * this case is likely rare in practice.
 *
 * @author Martin Davis
 *
 */
public class CascadedPolygonUnion {
    ...
    /**
     * Computes the union of a collection of
     * {@link Polygonal} {@link Geometry}s.
     *
    ...
    /**
     * The effectiveness of the index is somewhat sensitive to the node
     * capacity. Testing indicates that a smaller capacity is better. For an
     * STRtree, 4 is probably a good number (since this produces 2x2 "squares").
     */
     ...
     *
     * @return the union of the input geometries or null if no input geometries
     * were provided
    ...
    public Geometry union() {
         ...
         * geometries. This makes unioning more efficient, since vertices are
         * more likely to be eliminated on each round.
         */
...
//    return buffer0Union(geoms);
   ...
   * The following methods are for experimentation only
    ...
    //=======================================
    /**
     * Unions a list of geometries by treating the list as a flattened binary
     * tree, and performing a cascaded union on the tree.
     */
    ...
    /**
     * Unions a section of a list using a recursive binary union on each half of
     * the section.
     *
    ...
    /**
     * Gets the element at a given list index, or null if the index is out of
     * range.
     *
     * @param list
     * @param index
     * @return the geometry at the given index or null if the index is out of
     * range
     */
    ...
    /**
     * Reduces a tree of geometries to a list of geometries by recursively
     * unioning the subtrees in the list.
     *
    ...
    /**
     * Computes the union of two geometries, either or both of which may be
     * null.
     *
     * @param g0 a Geometry
     * @param g1 a Geometry
...
//  	return UnionInteracting.union(g0, g1);
     ...
     * intersection where possible. The case of MultiPolygons is optimized to
     * union only the polygons which lie in the intersection of the two
     * geometry's envelopes. Polygons outside this region can simply be combined
     * with the union result, which is potentially much faster. This case is
     * likely to occur often during cascaded union, and may also occur in real
     * world data (such as unioning data for parcels on different street
     * blocks).
     *
