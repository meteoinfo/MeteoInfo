 ...
 * {@link CoordinateReferenceSystem}. The coordinate is defined via X, Y, and
 * optional Z ordinates. Provides utility methods for comparing the ordinates of
 * two positions and for creating positions from Strings/storing positions as
 * strings. <p> The primary use of this class is to represent coordinate values
 * which are to be transformed by a {@link CoordinateTransform}.
 */
public class ProjCoordinate {
    ...
    /**
     * The X ordinate for this point. <p> Note: This member variable can be
     * accessed directly. In the future this direct access should be replaced
     * with getter and setter methods. This will require refactoring of the
     * Proj4J code base.
     */
    public double x;
    /**
     * The Y ordinate for this point. <p> Note: This member variable can be
     * accessed directly. In the future this direct access should be replaced
     * with getter and setter methods. This will require refactoring of the
     * Proj4J code base.
     */
    ...
    /**
     * Creates a ProjCoordinate using the provided double parameters. The first
     * double parameter is the x ordinate (or easting), the second double
     * parameter is the y ordinate (or northing), and the third double parameter
     * is the z ordinate (elevation or height).
     *
     * Valid values should be passed for all three (3) double parameters. If you
     * want to create a horizontal-only point without a valid Z value, use the
     * constructor defined in this class that only accepts two (2) double
     * parameters.
     *
     ...
     * Creates a ProjCoordinate using the provided double parameters. The first
     * double parameter is the x ordinate (or easting), the second double
     * parameter is the y ordinate (or northing). This constructor is used to
     * create a "2D" point, so the Z ordinate is automatically set to
     * Double.NaN.
     */
    ...
    /**
     * Create a ProjCoordinate by parsing a String in the same format as
     * returned by the toString method defined by this class.
     *
    ...
    /**
     * Sets the value of this coordinate to be equal to the given coordinate's
     * ordinates.
     *
     ...
     * ProjCoordinate provided as an ordinate is equal to the X ordinate value
     * of this ProjCoordinate. Because we are working with floating point
     * numbers the ordinates are considered equal if the difference between them
     ...
     * ProjCoordinate provided as an ordinate is equal to the Y ordinate value
     * of this ProjCoordinate. Because we are working with floating point
     * numbers the ordinates are considered equal if the difference between them
     * is less than the specified tolerance.
     */
     ...
     * ProjCoordinate provided as an ordinate is equal to the Z ordinate value
     * of this ProjCoordinate. Because we are working with floating point
     * numbers the ordinates are considered equal if the difference between them
     * is less than the specified tolerance.
     *
     * If both Z ordinate values are Double.NaN this method will return true. If
     * one Z ordinate value is a valid double value and one is Double.Nan, this
