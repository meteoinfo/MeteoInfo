 ...
 */
package org.meteoinfo.projection.proj4j.datum;
 ...
 * and a reference {@link Ellipsoid} giving an approximation to the true shape
 * of the geoid.
 * <p>
 * In order to transform between two geodetic points specified on different
 * datums, it is necessary to transform between the two datums. There are
 * various ways in which this datum conversion may be specified:
 * <ul>
 * <li>A 3-parameter conversion
 * <li>A 7-parameter conversion
 * <li>A grid-shift conversion
 * </ul>
 * In order to be able to transform between any two datums, the parameter-based
 * transforms are provided as a transform to the common WGS84 datum. The WGS
 * transforms of two arbitrary datum transforms can be concatenated to provide a
...
public class Datum {
    ...
    public static final Datum WGS84 = new Datum("WGS84", 0, 0, 0, Ellipsoid.WGS84, "D_WGS_1984");
    ...
    /**
     * Tests whether the datum parameter-based transform is the identity
     * transform (in which case datum transformation can be short-circuited,
    ...
    public void readEsriString(String esriString) {
        ...
        name = esriString.substring(iStart, iEnd);
        ellipsoid.readEsriString(esriString);
    }

    /**
     * To Esri string of datum part
     *
     * @return Esri string of datum part
     */
    public String toEsriString() {
        return "DATUM[\"" + name + "\"," + ellipsoid.toEsriString() + "]";
    }
}
