...
/**
 * Swiss Oblique Mercator Projection algorithm is taken from the USGS PROJ.4
 * package.
 */
public class SwissObliqueMercatorProjection extends Projection {
    ...
    public void initialize() {
        ...
        sp *= e;
        K = Math.log(Math.tan(ProjectionMath.FORTPI + 0.5 * phip0)) - c * (Math.log(Math.tan(ProjectionMath.FORTPI + 0.5 * phi0)) - hlf_e
                * Math.log((1. + sp) / (1. - sp)));
    ...
    public ProjCoordinate project(double lplam, double lpphi, ProjCoordinate xy) {
        ...
        sp = e * Math.sin(lpphi);
        phip = 2. * Math.atan(Math.exp(c * (Math.log(Math.tan(ProjectionMath.FORTPI + 0.5 * lpphi)) - hlf_e * Math.log((1. + sp) / (1. - sp)))
