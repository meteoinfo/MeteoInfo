 ...
 */
public class ListCartesianProductIterator<T> implements Iterator<List<T>> {
     ...
     */
    public ListCartesianProductIterator(List<List<T>> lists) {
        if (lists == null) {
    ...
    public List<T> next() {
        ...
        while (position >= 0) {
            ...
            if (indices[position] < lists.get(position).size()) {
                break;
            }
